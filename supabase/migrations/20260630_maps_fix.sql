-- Adiciona colunas que podem estar faltando na tabela maps existente.
-- Aditivo — sem DROP.

alter table public.maps add column if not exists image_url    text null;
alter table public.maps add column if not exists label        text not null default '';
alter table public.maps add column if not exists description  text not null default '';
alter table public.maps add column if not exists status       text not null default 'pending';
alter table public.maps add column if not exists index        integer not null default 0;
alter table public.maps add column if not exists metaforge_id text null;
alter table public.maps add column if not exists updated_at   timestamptz not null default now();
alter table public.maps add column if not exists created_at   timestamptz not null default now();

-- Tabela de marcadores (caso também não exista ainda)
...
-- Recria a policy (drop if exists antes para evitar conflito)
drop policy if exists "maps_select_all"        on public.maps;
drop policy if exists "map_markers_select_all" on public.map_markers;
